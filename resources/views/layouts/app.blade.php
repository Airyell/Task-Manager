    ...
    <nav id="sidebar" class="fixed top-0 left-0 h-screen w-[250px] bg-[#0b2c48] text-white overflow-y-auto p-4 z-[1045] transition-transform transform lg:translate-x-0 -translate-x-full lg:block">
      <div class="flex justify-between items-center mb-6 px-2">
        <a href="{{ route('dashboard') }}">
          <img src="{{ asset('assets/img/taskaroo-removebg.png') }}" class="w-full" alt="task manager" />
        </a>
      ...
      <ul class="space-y-2 px-2">
        <li>
          <a class="flex items-center px-4 py-2 rounded hover:bg-white/10 transition {{ request()->is('/') ? 'bg-white/10' : '' }}" href="{{ route('dashboard') }}">
            <i class="bi bi-house-door mr-2"></i> Home
          </a>
        </li>
        <li>
          <a class="flex items-center px-4 py-2 rounded hover:bg-white/10 transition {{ request()->is('projects*') ? 'bg-white/10' : '' }}" href="{{ route('projects.index') }}">
            <i class="bi bi-folder mr-2"></i> Projects
          </a>
        </li>
        <li>
          <a class="flex items-center px-4 py-2 rounded hover:bg-white/10 transition {{ request()->is('notes*') ? 'bg-white/10' : '' }}" href="{{ route('notes.index') }}">
            <i class="bi bi-sticky mr-2"></i> Notes
          </a>
        </li>
        <li>
          <a class="flex items-center px-4 py-2 rounded hover:bg-white/10 transition {{ request()->is('history*') ? 'bg-white/10' : '' }}" href="{{ route('history.index') }}">
      ...
      <header class="bg-[#0b2c48] text-white shadow sticky top-0 z-[1030]">
        <nav class="flex items-center justify-between px-4 py-3">
          <div class="flex items-center space-x-2">
            ...
            <a href="{{ route('dashboard') }}" class="text-white text-sm font-light" id="currentDateTime"></a>
          </div>
          @auth
            <div class="relative">
